...
export default function PassageResults({
  ...
  if (reference === "") {
    return <IonItem>Please enter a book reference!</IonItem>;
  } else if (!parseResult.parsed) {
    return <IonItem>Wrong format!</IonItem>;
  } else if (docSets?.filter((ds) => ds.document).length === 0) {
    return <IonItem>Book not found!</IonItem>;
  } else if (docSets?.filter((ds) => ds.document?.cv.length > 0).length === 0) {
    return <IonItem>Verse not found!</IonItem>;
    ...
    // by Verse
    if (
      !displayFlags[displayMode].allDocSets &&
      !displayFlags[displayMode].groupVerses
    ) {
      return docSets
      ...
      return <PassageByVerse cvArray={cvArray} docSets={docSets} it={it} />;
    } else {
      return null;
    }
  }
