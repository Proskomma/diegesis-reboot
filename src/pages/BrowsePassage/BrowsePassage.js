...
export default function BrowsePassage({ pkState, navState,it}) {
  ...
  const selectedDocSets =
    queryState.data.docSets?.filter(
      (ds) =>
        displayFlags[displayMode]?.allDocSets || ds.id === navState.docSetId
    ) || [];
